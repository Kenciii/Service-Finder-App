        ...
        <div className='p-1 '>
              <Menu.Item>
                {({ active }) => (
                  <Link
                    to={`${
                      user?.accountType ? "user-profile" : "serviceProvider-profile"
                    }`}
                    className={`${
                  ...
                  >
                    <CgProfile
                      ...
                      aria-hidden='true'
                    />
                    {user?.accountType ? "User Profile" : "Service Provider Profile"}
                  </Link>
                )}
              </Menu.Item>
...
const Navbar = () => {
  ...
  return (
  <>
    <div className='relative bg-[#00000026] z-50'>
      <nav className='container mx-auto flex items-center justify-between p-5'>
        <div>
          <Link to='/' className='text-black font-bold text-xl'>
            Service<span className='text-blue-500'>Finder</span>
          </Link>
        </div>
        ...
        <ul className='hidden lg:flex gap-10 text-base'>
          ...
          <li>
            <Link to= {user?.accountType === "seeker" ? "/applications":"/upload-service"}>
              {user?.accountType === "seeker" ? "Bookings":"Upload Service"}</Link>
          </li>
          <li>
      ...
      >
        ...
        <Link to='/serviceProvider' onClick={handleCloseNavbar}>
          Service Providers
        </Link>
        <Link
          onClick={handleCloseNavbar}
          to={
            user?.accountType === "seeker" ? "applications" : "upload-service"
          }
        >
          {user?.accountType === "seeker" ? "Bookings" : "Upload Service"}
        </Link>
