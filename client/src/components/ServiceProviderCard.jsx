  ...
  return (
    <div className='w-full h-16 flex gap-4 items-center justify-between bg-white shadow-md rounded'>
      <div className='w-3/4 md:w-2/4 flex gap-4 items-center'>
        <Link to={`/serviceProvider-profile/${cmp?._id}`}>
          <img
            src={cmp?.profileUrl}
            alt={cmp?.name}
        ...
        <div className='h-full flex flex-col'>
          <Link
            to={`/serviceProvider-profile/${cmp?._id}`}
      ...
      <div className='w-1/4 h-full flex flex-col items-center'>
        <p className='text-blue-600 font-semibold'>{cmp?.servicePosts?.length}</p>
